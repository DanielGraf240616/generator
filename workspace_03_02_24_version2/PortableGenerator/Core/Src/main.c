...
//Amplitude adjustment
static float Vout = 0;
static float wiperpos2 = 50; //Initial wiper position for 6 Vpp amplitude
static float digipotpos2 = 0;
static float wiperres2 = 0;

//Offset adjustment
float wiperpos1 = 127; //Initial wiper position for 0 V offset
float wiperres1;
float Voff;
...
float adc_voltage(uint32_t adc_value) //Calculate battery voltage
{
	float reference = 3.0;
...
float CalcAmplitude(float wiperres2) //Calculate amplitude value
{
...
float CalcOffset (float wiperres1) //Calculate offset
{
...
int main(void)
{
	...
	while (1)
	{
		...
		// DDS
		/* if ( direction != 0 && re_currentstate == 0)
		 {
