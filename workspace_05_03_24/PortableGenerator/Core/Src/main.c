...
static uint8_t res1 = 128; // Resistance for channel 1 (amplitude adjustment)
...
// ADC
	...
	float adc_voltage(float adc_value)
	{
		float reference = 3.0;
...
//	}
	...
	//Debounce function for rotary encoder
	void DebounceRotaryEcoder(void){
		static uint16_t debounce = 0xFFFF;
	...
	// Choosing closest resistance function
	float findClosest(float * array, size_t size, float target)
	{
		float closest = 0;
...
//	}
	...
	//Creating array of available resistance values of the TL0102 digital potentiometer
